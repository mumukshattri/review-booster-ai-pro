...
type FeedbackState = "loading" | "choose" | "confirm" | "form" | "submitting" | "thankyou" | "redirecting" | "error";
...
const FeedbackPage = () => {
  ...
  const loadCustomerData = async () => {
    try {
      ...
      // Mark sequence as stopped (customer engaged)
      try {
        await supabase.functions.invoke("feedback-data", {
          body: { customerId, action: "stop-sequence" },
        });
      } catch {}
  ...
  const handlePositive = () => setState("confirm");

  const redirectToGoogle = () => {
    setState("redirecting");
    // Mark as clicked and reviewed
    try {
      supabase.functions.invoke("feedback-data", {
        body: { customerId, action: "reviewed", reviewed: true },
      });
    } catch {}
    setTimeout(() => {
            ...
            {state === "confirm" && (
              <motion.div key="confirm" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.3 }} className={cardClass}>
                ...
                <div className="flex justify-center gap-1 mb-8">
                  {[0, 1, 2, 3, 4].map((i) => (
                    <motion.div key={i} initial={{ opacity: 0, scale: 0, rotate: -30 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ delay: 0.15 + i * 0.1, duration: 0.4, type: "spring", stiffness: 200 }}>
                      <Star className="w-10 h-10 text-amber-400 fill-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.4)]" />
                    </motion.div>
                  ))}
                </div>
                <button onClick={redirectToGoogle} className="w-full py-4 rounded-2xl text-base font-bold bg-primary text-primary-foreground hover:opacity-90 transition-opacity mb-3">
                  Yes, leave a review! ⭐
                </button>
                <button onClick={() => setState("thankyou")} className="text-sm text-muted-foreground hover:text-foreground transition-colors underline underline-offset-4">
                  No thanks
                </button>
              </motion.div>
            )}
